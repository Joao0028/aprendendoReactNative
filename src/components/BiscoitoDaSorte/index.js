...
export default function BiscoitoDaSorte() {
  ...
  const sorteia = Math.floor(Math.random() * frases.length);

  const [ estadoBiscoito, setBiscoito ] = useState(false);

  function sorteiaFrase(){
    return estadoBiscoito === false ? setBiscoito(true) : setBiscoito(false)
  }

  return (
    <View style={estilos.container}>
      <Image source={estadoBiscoito === false ? biscoito : biscoitoAberto} style={estilos.biscoitos} />
      <Text style={estilos.frase}>{estadoBiscoito === false ? "" : `"${frases[sorteia]}"`}</Text>
      <TouchableOpacity style={estilos.botao} onPress={sorteiaFrase}>
        <View>
          <Text style={{ color: "white", fontWeight: "bold", fontSize: 16 }}>
            {estadoBiscoito === false ? "Quebrar biscoito" : "Pegar outro"}
          </Text>
        </View>
      </TouchableOpacity>
    </View>
